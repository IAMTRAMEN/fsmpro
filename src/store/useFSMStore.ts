...
import { User, Technician, Customer, WorkOrder, Invoice, Provider, InvoiceSettings } from '../types';
...
interface FSMState {
  ...
  fetchInvoiceSettings: () => Promise<void>;
  fetchLiveTechnicianLocations: () => Promise<void>;
  
  addUser: (user: User | Technician) => Promise<void>;
  updateUser: (id: string, data: Partial<User | Technician>) => Promise<void>;
  ...
  assignTechnicians: (workOrderId: string, technicianIds: string[]) => Promise<void>;
  addWorkOrder: (workOrder: WorkOrder) => Promise<WorkOrder>;
  updateWorkOrder: (id: string, data: Partial<WorkOrder>) => Promise<void>;
  deleteWorkOrder: (id: string) => Promise<void>;
  
  addWorkOrderResource: (workOrderId: string, file: File, description?: string) => Promise<void>;
  deleteWorkOrderResource: (workOrderId: string, resourceId: string) => Promise<void>;
  addWorkOrderNote: (workOrderId: string, content: string) => Promise<void>;
  deleteWorkOrderNote: (workOrderId: string, noteId: string) => Promise<void>;

  updateTechnicianStatus: (id: string, status: string) => Promise<void>;
  ...
  fetchInvoices: async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const normalizedInvoices = invoices.map((inv: Invoice) => ({
          ...inv,
    ...
    try {
      ...
      if (res.ok) {
        set((state) => {
            ...
            ? [...state.technicians, user as Technician]
            : state.technicians;
    ...
    try {
      ...
      if (res.ok) {
        ...
        set((state) => {
          ...
          const newTechs = state.technicians.map(t => t.id === id ? updatedUser as Technician : t);
    ...
    try {
      ...
      if (res.ok) {
        ...
        set((state) => ({
          ...
          technicians: state.technicians.map(t => t.id === id ? updatedUser as Technician : t)
        }));
